    ...
    <div style={{ minHeight: '100vh', background: '#07070d', fontFamily: 'Plus Jakarta Sans,sans-serif' }}>
      <style>{`
        *{box-sizing:border-box}
        ...
        .sidebar-item{display:flex;align-items:center;gap:10px;padding:10px 14px;border-radius:10px;font-size:.85rem;font-weight:600;color:#71717A;cursor:pointer;transition:all .15s;text-decoration:none}
        .sidebar-item:hover{background:rgba(255,255,255,0.05);color:#E4E4E7}
        .sidebar-item.primary{background:linear-gradient(135deg,#6366F1,#818CF8);color:#fff;box-shadow:0 4px 16px rgba(99,102,241,0.3);margin-bottom:4px}

        /* DESKTOP */
        .sidebar{width:220px;height:100vh;position:fixed;top:0;left:0;background:rgba(255,255,255,0.02);border-right:1px solid rgba(255,255,255,0.06);display:flex;flex-direction:column;padding:20px 12px;z-index:100}
        .main{margin-left:220px;padding:28px 32px 60px}
        ...
        @media(max-width:768px){
          ...
          .main{margin-left:0!important;padding:16px 14px 100px!important}
          .grid2{grid-template-columns:1fr!important}
          .stats3{grid-template-columns:repeat(3,1fr)!important}
          .bottom-bar{display:flex!important;position:fixed;bottom:0;left:0;right:0;height:64px;background:rgba(7,7,13,0.97);border-top:1px solid rgba(255,255,255,0.08);backdrop-filter:blur(20px);z-index:200;align-items:center;justify-content:space-around;padding:0 8px}
          .mobile-header{display:flex!important;position:fixed;top:0;left:0;right:0;height:52px;background:rgba(7,7,13,0.95);border-bottom:1px solid rgba(255,255,255,0.06);backdrop-filter:blur(20px);z-index:200;align-items:center;justify-content:space-between;padding:0 16px}
          .mobile-extras{display:grid!important;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:12px}
        }
      `}</style>
      ...
      <div className="sidebar">
        ...
        <nav style={{ display: 'flex', flexDirection: 'column', gap: 2, flex: 1 }}>
          {SIDEBAR_ITEMS.map(item => (
            <Link key={item.href} href={item.href} className={`sidebar-item${item.primary ? ' primary' : ''}`}>
          ...
          {gam && (
            <div style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                <span>{gam.streak_atual >= 3 ? '🔥' : '⚡'}</span>
                <span style={{ fontSize: '0.85rem', fontWeight: 700, color: gam.streak_atual >= 3 ? '#f59e0b' : '#FAFAFA' }}>{gam.streak_atual} dias</span>
              ...
              <div style={{ fontSize: '0.62rem', color: '#52525B', marginTop: 4 }}>
                {metaOk ? '✓ Meta concluída' : `${gam.questoes_hoje || 0}/${gam.meta_diaria || 10} questões hoje`}
              </div>
            </div>
          )}
          <button onClick={sair} className="tap" style={{ width: '100%', padding: '8px', borderRadius: 8, fontSize: '0.75rem', color: '#52525B', background: 'none', border: '1px solid rgba(255,255,255,0.06)', cursor: 'pointer' }}>
            Sair
          </button>
        </div>
      </div>
      ...
      <div className="mobile-header">
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          {gam && <span style={{ fontSize: '0.8rem', color: gam.streak_atual >= 3 ? '#f59e0b' : '#71717A' }}>{gam.streak_atual >= 3 ? '🔥' : '⚡'}{gam.streak_atual}</span>}
      ...
      <div className="bottom-bar">
        {BOTTOM_NAV.map(item => (
          <Link key={item.href} href={item.href} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3, padding: '6px 12px', borderRadius: 10, textDecoration: 'none', flex: 1 }}>
            <span style={{ fontSize: '1.3rem' }}>{item.icon}</span>
            <span style={{ fontSize: '0.6rem', color: '#71717A', fontWeight: 600 }}>{item.label}</span>
          </Link>
        ))}
      </div>
      ...
      <div className="main" style={{ marginTop: 0 }}>
        ...
        {/* GREETING */}
        <div className="u" style={{ marginBottom: 20, paddingTop: 60 }}>
          <h1 style={{ fontSize: '1.4rem', fontWeight: 800, letterSpacing: '-0.03em', color: '#FAFAFA' }}>Olá, {nome} 👋</h1>
          <p style={{ color: '#52525B', fontSize: '0.8rem', marginTop: 3 }}>
        ...
        {/* STREAK HERO */}
        {gam && (
          <div className="u" style={{ ...card, padding: '18px 20px', marginBottom: 16, background: gam.streak_atual >= 3 ? 'linear-gradient(135deg,rgba(245,158,11,0.1),rgba(239,68,68,0.05))' : 'rgba(255,255,255,0.03)', border: gam.streak_atual >= 3 ? '1px solid rgba(245,158,11,0.25)' : '1px solid rgba(255,255,255,0.07)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 16, flexWrap: 'wrap' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
                <CircleProgress pct={Math.min(100, gam.streak_atual * 3.3)} size={58} stroke={5} color={gam.streak_atual >= 7 ? '#f59e0b' : gam.streak_atual >= 3 ? '#fb923c' : '#6366F1'}>
                  <span style={{ fontSize: '1.2rem' }}>{gam.streak_atual >= 3 ? '🔥' : '⚡'}</span>
                </CircleProgress>
                <div>
                  <div style={{ fontSize: '1.8rem', fontWeight: 900, letterSpacing: '-0.04em', lineHeight: 1, color: gam.streak_atual >= 3 ? '#f59e0b' : '#FAFAFA' }}>
                    {gam.streak_atual}<span style={{ fontSize: '0.8rem', fontWeight: 400, color: '#52525B', marginLeft: 5 }}>dias</span>
                  </div>
                  <div style={{ fontSize: '0.68rem', color: '#52525B', marginTop: 3 }}>
                    🏆 Recorde: {gam.streak_maximo} · ⚡ {gam.pontos} pts
                    {gam.streak_freeze > 0 && <span style={{ color: '#818CF8' }}> · 🛡️{gam.streak_freeze}</span>}
                  </div>
                </div>
              </div>
              ...
              <button onClick={handleFreeze} disabled={comprando || gam.pontos < 50} className="tap"
                style={{ fontSize: '0.68rem', padding: '7px 12px', borderRadius: 9, whiteSpace: 'nowrap', background: gam.pontos >= 50 ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.03)', color: gam.pontos >= 50 ? '#818CF8' : '#3F3F46', border: `1px solid ${gam.pontos >= 50 ? 'rgba(99,102,241,0.25)' : 'rgba(255,255,255,0.05)'}`, cursor: gam.pontos >= 50 ? 'pointer' : 'default' }}>
                🛡️ Freeze
              </button>
            </div>
            {freezeMsg && <div style={{ fontSize: '0.7rem', color: '#818CF8', marginTop: 8, textAlign: 'center' }}>{freezeMsg}</div>}
          </div>
        )}
        ...
        <div className="grid2">
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div className="u stats3">
              {[
                ...
                { l: 'Maior gap', v: piorArea ? AREA_ICONS[piorArea.area] : '—', s: piorArea ? piorArea.area.split(' ').pop() || '' : '—', c: '#F87171' },
              ].map((s, i) => (
                <div key={i} style={{ ...card, padding: '12px 10px' }}>
                  <div style={{ fontSize: '0.57rem', color: '#3F3F46', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 8 }}>{s.l}</div>
                  <div style={{ fontSize: '1.4rem', fontWeight: 900, color: s.c, lineHeight: 1 }}>{s.v}</div>
                  <div style={{ fontSize: '0.62rem', color: '#52525B', marginTop: 5 }}>{s.s}</div>
                </div>
              ))}
            </div>
            ...
            {/* DESEMPENHO */}
            {desempenho.length > 0 && (
              <div className="u" style={{ ...card, padding: '16px 18px' }}>
                <div style={{ fontSize: '0.6rem', color: '#52525B', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 14 }}>Desempenho por área</div>
                {desempenho.map(d => {
                  const c = AREA_COLORS[d.area] || '#818CF8'
                  return (
                    <div key={d.area} style={{ marginBottom: 12 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                          <span style={{ fontSize: '0.88rem' }}>{AREA_ICONS[d.area]}</span>
                          <span style={{ fontSize: '0.83rem', fontWeight: 600, color: '#E4E4E7' }}>{d.area}</span>
                        </div>
            ...
            <div className="mobile-extras">
              {[
                ...
                <Link key={a.href} href={a.href}>
                  <button className="tap" style={{ width: '100%', padding: '12px 6px', borderRadius: 12, position: 'relative', background: a.accent ? 'rgba(251,191,36,0.08)' : 'rgba(255,255,255,0.03)', border: a.accent ? '1px solid rgba(251,191,36,0.2)' : '1px solid rgba(255,255,255,0.07)', color: a.accent ? '#FBBF24' : '#71717A', fontSize: '0.7rem', fontWeight: 600, cursor: 'pointer' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div className="u" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <Link href="/simulado">
                <button className="tap" style={{ width: '100%', padding: '16px', borderRadius: 14, fontWeight: 800, fontSize: '0.9rem', border: 'none', cursor: 'pointer', background: 'linear-gradient(135deg,#6366F1,#818CF8)', color: '#fff', boxShadow: '0 6px 20px rgba(99,102,241,0.3)' }}>
                  🚀 {totalQuestoes === 0 ? 'Primeiro simulado' : 'Novo simulado'}
                </button>
              </Link>
              <Link href="/redacao">
                <button className="tap" style={{ width: '100%', padding: '16px', borderRadius: 14, fontWeight: 700, fontSize: '0.9rem', cursor: 'pointer', color: '#E4E4E7', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)' }}>
                  ✍️ Treinar redação
                </button>
              </Link>
            </div>
            ...
            {badgesOk.length > 0 && (
              <div className="u" style={{ ...card, padding: '16px 18px' }}>
                <div style={{ fontSize: '0.6rem', color: '#52525B', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 12 }}>Conquistas ({badgesOk.length}/{BADGES.length})</div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 7 }}>
                  {badgesOk.map(b => (
                    <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 100, background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.2)' }}>
                  ...
                  {BADGES.filter(b => !gam?.badges?.includes(b.id)).slice(0, 2).map(b => (
                    <div key={b.id} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '4px 10px', borderRadius: 100, background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', opacity: 0.35 }}>
            ...
            {simulados.length > 0 && (
              <div className="u" style={{ ...card, padding: '16px 18px' }}>
                <div style={{ fontSize: '0.6rem', color: '#52525B', textTransform: 'uppercase', letterSpacing: '0.1em', fontWeight: 600, marginBottom: 12 }}>Últimos simulados</div>
                {simulados.map((s, i) => (
                  <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: i < simulados.length - 1 ? '1px solid rgba(255,255,255,0.05)' : 'none' }}>
                    <div>
                      <div style={{ fontSize: '0.8rem', color: '#A1A1AA', fontWeight: 500 }}>{s.total_questoes} questões</div>
                      <div style={{ fontSize: '0.62rem', color: '#3F3F46', marginTop: 2 }}>{new Date(s.created_at).toLocaleDateString('pt-BR')}</div>
                    </div>
